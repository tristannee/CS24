...
ThreadContext *__sthread_scheduler(ThreadContext *context) {
  /* We consider the case when no context is inputted (initial case) */
  if (context == NULL) {
      /* If the ready queue is empty, we must exit */
      if (queue_empty(&ready_queue) == 1) {
          /* If the blocked queue is empty, exit(0) */
          if (queue_empty(&blocked_queue) == 1)
	  {
	      printf("Exited Successfully\n");
	      exit(0);
	  }
	  /* If the blocked queue is not empty, exit(1) */
          else
	  {
	      printf("Failure In Exiting\n");
	      exit(1);
	  }    
      }
      /* If not, then we take from the queue and set it running */
      else
      {
  ...
  /* We consider the case when context is inputted */
  else
  {
      /* We set the current context to input context */     
      current->context = context;
      /* If the thread is finished, delete the thread */
      if (current->state == ThreadFinished) {
          __sthread_delete(current);
      }
      /* If the thread is blocked, add to blocked queue */
      else if (current->state == ThreadBlocked) {
          queue_add(current);
      }
      /* If the thread is running, add to queue and set ready */
      else if (current->state == ThreadRunning){
          current->state = ThreadReady;
          queue_add(current);
      }
      /* If the queue is not empty, take queue and set running */
...
Thread * sthread_create(void (*f)(void *arg), void *arg) {
    /* Initialize a new memory pointer */
    void *newMemory = malloc(DEFAULT_STACKSIZE);
    /* Initialize a new thread pointer */
    Thread *newThread = malloc(sizeof(Thread));
    /* Set thread's memory as allocated memory's pointer */
    newThread->memory = newMemory;
    /* Set thread's state as ready */
    newThread->state = ThreadReady;
    /* Initialize context from top of stack, function, and argument */
    newThread->context =
      __sthread_initialize_context((unsigned char *)
				   newMemory + DEFAULT_STACKSIZE, f, arg);
 ...
 */
void __sthread_delete(Thread *threadp) {
    /* We free the memory pointer */
    free(threadp->memory);
    /* We free the thread pointer */
    free(threadp);
}
